...
class LaneTracker(LaneUI):
    ...
    def lane_association(self):
        ...
        lm_list = self.get_candi_lanes(self.cur_frame) #yzm 旧的历史 lines，应该是世界坐标系w下的
        det_list = self.cur_frame.get_lane_features() #yzm 新的 lines
        self.assoc_model.set_landmark(lm_list)
        self.assoc_model.set_deteciton(det_list, self.cur_frame.T_wc) #对新的线进行旋转平移，self.cur_frame.T_wc表示从相机坐标系(c)到世界坐标系(w)的变换
        A, stats = self.assoc_model.association()
        # if cfg.debug_flag:
        #     self.visualize_association(lm_list, det_list, self.cur_frame.T_wc, A=A)
        A = np.array(A).reshape(-1, 2) #A是最终匹配结果，第一列是lm的索引，第二列是det的索引
        #结果更新的是det_list， det_list中的线被赋予id
        for i in range(len(det_list)):
            if i in A[:, 1]: #索引在匹配结果中， [:, 1]取第一列det_list， 第0列是lm_list
                j = A[A[:, 1] == i, 0][0] # 找到对应的地图线索引j，会对A的第1列的每个元素，如果等于i，则取对应的第0列元素作为j
                det_list[i].id = lm_list[j].id # 继承地图线j的ID
            elif det_list[i].self_check(): #过滤长度小于2m的线， 对于合格的没匹配上的线，分配一个新的ID
