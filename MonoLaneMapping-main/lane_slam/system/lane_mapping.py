...
class LaneMapping(LaneOptimizer):
    ...
    def process(self):
        for frame_id, frame_data in enumerate(tqdm.tqdm(self.frames_data, leave=False, dynamic_ncols=True)):
            ...
            print("pose_wc: ", pose_wc)
            timestamp = frame_data['timestamp']
            lane_pts_c = drop_lane_by_p(deepcopy(frame_data['lanes_predict']), p=cfg.preprocess.drop_prob) #lane_pts_c表示cam坐标下的线
            self.time_stamp.append(timestamp)


            print("lane_pts_c: ", lane_pts_c)
            # 1. odometry
            t0 = perf_counter()
            ...
            # 2. lane association
            t1 = perf_counter()
            self.lane_association() #更新了self.lane_features； lm的线在lanes_in_map中， det的线在self.lane_features中
