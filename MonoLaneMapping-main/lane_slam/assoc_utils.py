...
def affinity2assoc(affinity, method='default'):
    ...
    n, m = affinity.shape #n是lm的数量，m是det的数量
    if method == 'km':
        if n > m:
            affinity = np.concatenate([affinity, np.zeros((n, n - m))], axis=1) # 添加零列，使矩阵变为方阵 (n x n)
        ...
        #     if affinity[i, j] > 0:
        #         A.append([i, j])
        for det_id in range(m): # 遍历每条检测线
            lane_id = np.argmax(affinity[:, det_id]) # 找到与该检测线匹配分数最高的历史线
...
def left_or_right(lane_a, lane_b):
    ...
    point_on_b1 = lane_b[0, :2] #取起点的 x,y, [0, :2]即取第一行的前两列
    point_on_b2 = lane_b[lane_b.shape[0]-1, :2] #取起点的 x,y
...
class KnnASSOC:
    ...
    def association(self):
        ...
        for i in range(self.num_det):
            xyz = self.det_feature[i] #每条新的检测的线
            distance_thd = self.det_knn_thd[i] #每条新的检测的线对应的距离阈值
            dist_candidates = [] # store all distance candidates
            for j in range(self.num_lm):
                if self.lm_categories[j] != self.det_categories[i]: #类别不匹配跳过
                    continue
                dist, idx = self.lm_kdtrees[j].query(xyz, k=1) #xyz: 形状为(N_i, 3)，第i条检测线的N_i个三维点； 在第j条历史线的kd树中寻找最近邻
                # dist: 形状为(N_i, 1)，表示第i条检测线的N_i个三维点到第j条历史线的最近邻的距离
                # idx: 形状为(N_i, 1)，表示第i条检测线的N_i个三维点到第j条历史线的最近邻的索引
                if self.knn_type == 'lmr':#使用lmr特征时，重新计算距离,距离为欧式距离
                    dist = np.linalg.norm(xyz[:, :self.dim] - self.lm_kdtrees[j].data[idx, :self.dim], axis=1)
                dist_candidates.append(dist.reshape(-1, 1)) #从一维数组到列向量，主要为了调试吧
                dist_match = dist[dist < distance_thd] #拿到距离在阈值内的，阈值差不多是1.5
                if len(dist_match) == 0:
                    continue #没有匹配上，跳过
                # compute association score ↓
                #score = 平均匹配距离 × sqrt(总点数 / 匹配点数)
                  #mean(dist_match) → 匹配点的平均距离，越小越好
                  #sqrt(N_i / N_match) → 匹配比例惩罚项, 如果全部匹配：sqrt(N_i/N_i) = 1，不惩罚, 匹配比例越低，惩罚越大
                score = np.mean(dist_match) * np.sqrt(len(dist) / len(dist_match)) #这个score是一个标量，表示lm线和det线的匹配得分
                
                #定义一个"理论上足够好"的阈值。= 平均距离阈值 × sqrt(1 / 最小匹配比例)
                ideal_dist = np.mean(distance_thd) * np.sqrt(1 / self.min_match_ratio)
                assoc_dist[j, i] = score
                assoc_dist_thd[j, i] = ideal_dist
                #最终得分assoc_scores， 越大越好
                assoc_scores[j, i] = 1 / score if score < ideal_dist else 0

        #几何一致性增强，利用多条车道线之间的相对位置关系验证匹配， lm中的相对位置关系和 det中的相对位置关系是否一致，想象三条平行车道线：左、中、右。无论车辆如何移动，它们的相对位置关系保持不变
    ...
    def construct_consistency(self, affinity):
        ...
        for i in range(self.num_lm):
            for j in range(self.num_det):
                if affinity[i, j] > 0:
                    A.append([i, j]) #例如 A = [[0,1], [0,2], [1,0], [1,1], ...]， 放lm和det的索引， 这个结果已经是有可能匹配上的
        w = 1
        for aij in A:
            i, j = aij
            for bkl in A:
                ...
                if i == k or j == l:
                    ...
                    # lm线即旧的历史线，计算左右关系和距离
                    a, dist_valid_a = left_or_right(self.lm_xyz[i][-self.det_xyz[j].shape[0]:, :self.dim], #取lm第i条线的最后N个点，N为det第j条线的点数， dim为2则取x,y， 3则取x,y,z
                                                    self.lm_xyz[k][-self.det_xyz[l].shape[0]:, :self.dim]) #取lm第k条线的最后N个点，N为det第l条线的点数， dim为2则取x,y， 3则取x,y,z, 
                    # det线即新线，计算左右关系和距离
                    b, dist_valid_b = left_or_right(self.det_xyz[j], self.det_xyz[l])
                    if a == b: #方向一致
                        A_consistency[i, j] += w / (1 + np.abs(dist_valid_a - dist_valid_b))
        #总结下， 就是对于匹配结果A， lm的第i条和det的第j条能匹配，那么遍历所有的匹配结果，lm的i在其他组的lm的左边还是右边，和i匹配的det的j在其他组的det的左边还是右边，如果一致，说明i和j的相对位置关系是一致的，增加一致性分数
