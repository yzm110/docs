...
class LinkedPoints(object):
    ...
    def get_nearest_ctrlpts4(self, point):
        ...
        if near_enough and not is_first and not is_last:
            ...
            return None
    def find_footpoint(self, point): #找到point在曲线上的垂足， point是det的点， line是对应的lm线即det的控制点
        point = point[np.newaxis, :3]
        dist, idx = self.kdtree.query(point, k=2) #dist：形状(1,2)，点到最近两个控制点的距离；  idx：形状(1,2)，最近两个控制点的索引
        if (dist > cfg.lane_mapping.ctrl_points_chord).any(): #距离太远
            return None, None, None
        if (idx == 0).any() or (idx == (self.size() - 1)).any(): #最近点是第一个或最后一个控制点
            return None, None, None
        if abs(idx[0][0] - idx[0][1]) > 1: #两个最近点不是相邻控制点
            return None, None, None
        idx1 = min(idx[0])
        idx2 = max(idx[0])
        ...
        u, error = parameterization(point, ctrl_pts_np) #u：样条段内的参数（通常归一化到 [0,1]，u 表示点在 ctrl_pts[1]→ctrl_pts[2] 段上的位置），
                                                          #error：点到样条最近点的欧氏距离（残差）。
        if u is None:
            return None, None, None
