...
def error_catmull_rom(measurement: np.ndarray, this: gtsam.CustomFactor,
    ...
    ctrl_pts = [values.atPoint3(this.keys()[i]) for i in range(4)] #取出控制点
    ctrl_pts = np.concatenate(ctrl_pts, axis=0).reshape(4,3)
    spline = CatmullRomSpline(ctrl_pts)#构造样条
    u = measurement[3]

    est_pt, coeff = spline.get_point(u, return_coeff=True)
    #est_pt: 样条在 u 处的三维点，shape (3,)； u是pt_w在样条上的垂足，在ctrl_pts[1],ctrl_pts[2]之间的位置
