...
class LaneFeature:
    ...
    def __init__(self, id, points, category):
        ...
        self.ctrl_points_chord = cfg.lane_mapping.ctrl_points_chord # val=3
    ...
    def init_ctrl_pts(self, lane_w, cur_pose_cw):
        ...
        self.get_skeleton(lane_w.get_xyzs(), polyline = lane_w.polyline) # 最终得到 self.ctrl_pts， 是一段在原始点基础上，抽稀的点集，这个点集能表达原来线的几何形状
    ...
    def get_pts_to_add(self, points): #只保留那些与当前控制点方向一致的点，过滤掉"后方"或"侧方"的点。
        # points: N x 3； 线的离散点集
        ctrl_pts_size = self.ctrl_pts.size()
        if ctrl_pts_size < 2:
            return points
        else:
            normal_a = (self.ctrl_pts.get_xyz(0) - self.ctrl_pts.get_xyz(1)) #从第二个控制点指向第一个控制点的向量（头部方向）
            normal_b = (self.ctrl_pts.get_xyz(-1) - self.ctrl_pts.get_xyz(-2)) #从倒数第二个控制点指向最后一个控制点的向量（尾部方向）
        pts_to_add = []
        for pt in points:
            d_a = pt - self.ctrl_pts.get_xyz(0) #点到第一个控制点的向量
            d_b = pt - self.ctrl_pts.get_xyz(-1) #点到最后一个控制点的向量
            cos_a = np.dot(d_a, normal_a) / (np.linalg.norm(d_a) * np.linalg.norm(normal_a)) #点与第一个控制点方向的余弦值
            cos_b = np.dot(d_b, normal_b) / (np.linalg.norm(d_b) * np.linalg.norm(normal_b)) #点与最后一个控制点方向的余弦值
            thd = np.cos(np.deg2rad(cfg.lane_mapping.skeleton_angle_thd))
            if cos_a > thd or cos_b > thd: ## 条件：例如 夹角 < 60°（余弦值 > 0.5）
    ...
    #get_skeleton 是从一条稠密的 lane 点云（origin_points）中迭代抽稀、生成控制点链（self.ctrl_pts）的核心算法；
    # 它以固定半径（ctrl_points_chord）为尺度，优先选取“边界/外侧”点并在需要时用拟合 polyline 作引导来生成新的控制点，从而保证控制点间距和曲线拓扑。
    def get_skeleton(self, origin_points:np.ndarray, inital_point=None, polyline=None):
        ...
        #原始点集 → 方向过滤 → 边界搜索 → 点集更新 → 重复...
        while True:
            num += 1
            # if self.id == 2 and self.ctrl_pts.size() >= 43:
            #     self.id = self.id
            origin_points = self.get_pts_to_add(origin_points) # 过滤掉那些与当前控制点方向不一致的点，抽稀效果
            ...
            #inner_border:距离inital_point最远的但在ctrl_points_chord（3m）范围内的点
            #outer_border:距离inital_point最近的但在ctrl_points_chord（3m）范围外的点
            if outer_border is None: # this is equal to the case that no_assigned is empty
                # 如果 outer_border is None（即没有超出半径的点，no_assigned 可能空）：
                # 说明当前覆盖区没有需要继续扩展的外点 → 算法尝试在 head 或 tail 方向“填充/延伸”一个新控制点，然后结束（return True）。
                # 选择靠近 inner_border 的端（head 或 tail）比较 d_head, d_tile（到当前 ctrl 首/尾的距离）。
                # 若 ctrl_pts.size() >= 2，会先用 get_query()（基于 polyline）得到一个更合理的 inner_border（用于引导）。
                # 然后通过 get_next_node(inner_border, center, chord, polyline) 计算下一个控制点（见后面），把它 add 或 append 到 ctrl_pts，最后返回 True（结束）。                
                # # add（往头部插入）或 append（往尾部）。!!!!!!!!
                ...
                # 比较 outer_border 到 head/tail 的距离，决定把下一个控制点放在链头还是链尾。
                # 若需要，用 get_query() 用 polyline 修正 outer_border（当已有至少两个 ctrl 点时）。
                # 计算 next_initial = get_next_node(outer_border, chosen_center, chord, polyline, points_debug)。然后把 next_initial 插入（add 或 append）。
                # 把 inital_point 更新为 next_initial，并把 origin_points 除去已被分配的点 origin_points = origin_points[no_assigned]，继续循环。
